...
-- pgQueryB [sqlExp| SELECT id, name FROM users WHERE name = #{userName}|]
-- @
pgQueryB :: (HasPostgres m, FromRow r)
         => SqlBuilder -> m [r]
pgQueryB bld = withPostgresIO $ \c -> do
    b <- sqlBuild bld c
    query_ c $ Query $ toByteString b
...
-- | Same as 'pgQueryB' but returns count of affected rows like 'pgExecute'
-- does
pgExecuteB :: (HasPostgres m)
           => SqlBuilder -> m Int64
pgExecuteB bld = withPostgresIO $ \c -> do
    b <- sqlBuild bld c
    execute_ c $ Query $ toByteString b
