...
-- exceptions. If current monad is already in transaction then run given action
-- in savepoint, commit savepoint on success, rollback on error.
pgAtomically
  :: forall n m a
   . ( MonadBaseControl IO n
     , MonadTransaction n m, KnownNat (ToNat (TransactionLevel m)) )
  => n a
  -> m a
pgAtomically action = do
  con <- askConnection
  let level = natVal (Proxy :: Proxy (ToNat (TransactionLevel m)))
  if level == 0 then runTrans con else runSP con
  where
